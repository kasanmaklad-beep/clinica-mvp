...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const updated = await prisma.dailyReport.update({
    ...
    data: {
      ...
      editadoPorId: userId,
      consultas: { create: consultas.filter(c => c.numPacientes > 0 || c.totalBs > 0 || c.ingresoDivisa > 0 || c.efectivoUsd > 0) },
      servicios: { create: servicios.filter(s => s.numPacientes > 0 || s.totalBs > 0 || s.ingresoDivisa > 0 || s.efectivoUsd > 0) },
