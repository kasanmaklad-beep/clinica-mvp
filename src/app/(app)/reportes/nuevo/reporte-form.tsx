...
interface AnticipoRow {
  ...
  _nuevaAseg: string;   // campo temporal para crear nueva aseguradora inline
}
interface CuentaRow {
...
function NumInput({ value, onChange, decimal = false, className = "" }: {
  value: number; onChange: (n: number) => void; decimal?: boolean; className?: string;
}) {
  return (
    <input
      type="number"
      ...
      className={`h-9 w-full rounded-lg border border-[var(--border)] bg-[var(--card)] px-2 text-sm text-right focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]/40 ${className}`}
      placeholder="0"
    />
  );
}

// Dropdown de aseguradora con opción "Nueva…"
  ...
  return (
    <div className="space-y-1.5">
      <select
        value={value}
        onChange={(e) => {
          if (e.target.value === "__nueva__") setNombre("");
          else onChange(e.target.value);
        }}
        className="h-9 w-full rounded-lg border border-[var(--border)] bg-[var(--card)] px-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]/40"
      >
...
export function ReporteForm({ especialidades, unidades }: {
  especialidades: Especialidad[]; unidades: UnidadServicio[];
}) {
  ...
  async function crearAseguradora(nombre: string): Promise<Aseguradora | null> {
    ...
    setAseguradoras(prev => {
      if (prev.find(a => a.id === data.aseguradora.id)) return prev;
      return [...prev, data.aseguradora].sort((a, b) => a.nombre.localeCompare(b.nombre));
    });
  ...
  // Pacientes por área
  const [pacientesArea] = useState<PacienteAreaRow[]>([
    { area: "EMERGENCIA", numPacientes: 0 },
    { area: "HOSPITALIZACION", numPacientes: 0 },
    { area: "UCI", numPacientes: 0 },
  ]);
  const [pacsArea, setPacsArea] = useState(pacientesArea);
  ...
  // APS
  const [aps, setAps] = useState({ consultas: 0, laboratoriosImagenes: 0, movimientosDia: 0, totalFacturados: 0, noFacturadosComentarios: "", facturadosComentarios: "" });
  ...
  async function guardar(estado: "BORRADOR" | "CERRADO") {
    ...
    setSaving(false);
    if (!res.ok) {
      setError(res.status === 409 ? `Ya existe un reporte para el ${fecha}. Editalo desde el Histórico.` : (data.error || "Error al guardar"));
      return;
    }
  ...
  return (
    <div className="space-y-4 pb-28">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Nuevo reporte</h1>
          <p className="text-[var(--muted-foreground)] mt-1 text-sm">Ingresos de Caja Diario — Hospital Clínicas del Este</p>
        </div>
      </div>

      {error && <div className="rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 p-3 text-sm text-[var(--destructive)]">{error}</div>}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="tasa" type="number" inputMode="decimal" min={0} step="0.01"
              value={tasa || ""} onChange={e => setTasa(parseFloat(e.target.value) || 0)} placeholder="Ej: 92.50" />
          </div>
          ...
          {especialidades.map((esp, idx) => {
            ...
            return (
              <div key={esp.id} className={`rounded-lg border p-3 transition-colors ${hasData ? "border-blue-200 bg-blue-50/50 dark:border-blue-900/50 dark:bg-blue-950/20" : "border-[var(--border)]"}`}>
                <div className="sm:hidden text-sm font-medium mb-2">
                  {esp.nombre}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-[2.5fr_1fr_1.2fr_1fr_1fr] gap-2 items-center">
                  ...
                  <div className="space-y-1">
                    <Label className="sm:hidden text-xs">Pacientes</Label>
                    <NumInput value={row.numPacientes} onChange={v => updateConsulta(idx, "numPacientes", v)} />
                  </div>
                  <div className="space-y-1">
                    <Label className="sm:hidden text-xs">Total Bs.</Label>
                    <NumInput value={row.totalBs} onChange={v => updateConsulta(idx, "totalBs", v)} decimal />
                  </div>
                  <div className="space-y-1">
            ...
            <div key={idx} className="rounded-lg border border-[var(--border)] p-3 space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <select value={ant.tipo}
                    onChange={e => setAnticipos(prev => { const n = [...prev]; n[idx] = { ...n[idx], tipo: e.target.value as AnticipoRow["tipo"] }; return n; })}
                ...
                <div className="space-y-1.5">
                  ...
                  <NumInput value={ant.totalBs} decimal onChange={v => setAnticipos(prev => { const n = [...prev]; n[idx] = { ...n[idx], totalBs: v }; return n; })} />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="sm:col-span-2 space-y-1.5">
                  ...
                  <AseguradoraSelect
                    value={ant.aseguradoraId}
                    onChange={id => setAnticipos(prev => { const n = [...prev]; n[idx] = { ...n[idx], aseguradoraId: id }; return n; })}
                    aseguradoras={aseguradoras}
                    onNew={crearAseguradora}
                  />
                </div>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:left-64 z-20 bg-[var(--card)]/95 backdrop-blur border-t border-[var(--border)] p-3 flex gap-2 justify-end">
        <Button variant="outline" onClick={() => guardar("BORRADOR")} disabled={saving}>
          <Save className="h-4 w-4" />{saving ? "..." : "Borrador"}
        </Button>
